...
function booksByAuthor(dictionary) {
  // Your code here:
  let booksArray = [];
  let authorKeys = Object.keys(dictionary);
  authorKeys.forEach(author => {
    dictionary[author].forEach(books => {
      booksArray.push({
        title: books[0],
        pages: books[1],
        author: author
...
function averagePageCount(books) {
  ...
  books.forEach(element => {
    averagePageCount += element.pages;
  })

  return averagePageCount / books.length;
