...
function booksByAuthor(dictionaryObject) {
  ...
  for (key in dictionaryObject){
    for (i of dictionaryObject[key]){
      authorObject = {};
      authorObject.title = i[0];
      authorObject.pages = i[1];
      authorObject.author = key;
      arrayBooks.push(authorObject);
    };
  };
  return arrayBooks;
};
...
function averagePageCount(arrayBooks) {
  ...
  averageTimePage = Math.ceil(numberPages/arrayBooks.length);
  return averageTimePage;
};