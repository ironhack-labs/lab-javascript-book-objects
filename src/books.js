...
function booksByAuthor(dictionary) {
  const newBookByAuthor = [];

  const autors = Object.keys(dictionary);

  for (let autor of autors) {
    const books = dictionary[autor];

    for (let book of books) {
      console.log(book);
      newBookByAuthor.push({
        title: book[0],
        pages: book[1],
        author: autor,
      });
...
function averagePageCount(books) {
  ...
  const average = count / books.length;
  //console.log(average);
  return average;
}
