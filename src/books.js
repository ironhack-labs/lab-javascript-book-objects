...
const allBookDetails = booksArray.map(getBookDetails);
...
function averagePageCount(dictionary) {
  ...
  for (let author in dictionary) {
    if (Array.isArray(dictionary[author])) {
      dictionary[author].forEach(([title, pages]) => {
        totalPages += pages;
        totalBooks += 1;
      });
    }
  ...
  const average = totalPages / totalBooks;


  return {
    average: average.toFixed(2),
    operation: `${totalPages} / ${totalBooks}`,
  };
}