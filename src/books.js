...
function booksByAuthor(dictionary) {
  ...
  for (let i = 0; i < authors.length; i++) {
    ...
    for (let j = 0; j < dictionary[author].length; j++) {
      const dic = {
        title: dictionary[author][j][0],
        pages: dictionary[author][j][1],
        author: author,
      };
      booksArr.push(dic);
    }
...
const booksDic = booksByAuthor(dictionary);
console.log(booksDic);
...
function averagePageCount(arr) {
  ...
  return pageCount / arr.length;
}

console.log(averagePageCount(booksDic));
