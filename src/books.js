...
const result = getBookDetails(book2);
console.log(result);
...
for (let i = 0; i < booksArray.length; i++) {
  ...
  booksArray[i].readingTime = Math.round((pages * 500) / 90);
}
...
function createBooks(dictionary) {
  const books = [];

  for (const author in dictionary) {
    const booksByAuthor = dictionary[author];
...
const result = createBooks(dictionaryExample);
console.log(result);
